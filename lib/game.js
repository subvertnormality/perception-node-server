...
function generateNewRound(done) {
  ...
  currentGame((err, g) => {
    ...
    locations.data.roundType = 'default';
    console.log('current round ', currentRound.data.playerType)
    locations.data.playerType = currentRound.data.playerType === 'hero' ? 'twitch' : 'hero';
    console.log('new round ', locations.data.playerType)
    done(err, locations);
  });
...
function startNewRound(done) {
  currentGame((err, g) => {
    generateNewRound((err, round) => {
      g.p('round', round);
      if (round.data.playerType == 'twitch') {
        queue.setToTwitchMode(() => {
          g.save((err) => {
            done(err, true);
          });
        })
...
function attemptUnlockRound(attempt, done) {
  ...
  currentGame((err, g) => {
    ...
    if (_.trim(_.lowerCase(round.data.name)) == _.trim(_.lowerCase(attempt))) {
      ...
      logger.info('Incorrect guess for clue :' + round.data.name + ' | Guess was: ' + attempt)
      done(err, false);
    }
...
function playerMode(cb) {
  currentGame((err, g) => {
    const round = g.p('round');
    if (err || !round.data.playerType) {
      cb(err, false);
    } else {
      cb(null, round.data.playerType);
    }
  });
}
...
module.exports.playerMode = playerMode;