...
function init(io, sessionSettings) {
  ...
  function onAuthorizeSuccess(socket, data, accept) {
    ...
    if (existingSocketId && io.sockets.connected[existingSocketId]) {
      io.sockets.connected[existingSocketId].emit('halt')
      io.sockets.connected[existingSocketId].disconnect();
      rpcStub.handleResetEvent({}, function (error, img) {});
    }
  ...
  io.on('connection', function (socket) {
    ...
    connectedUserActivityCheck = setInterval(function activityCheck() {
      ...
      if (connectedUserLastEventTime && Math.floor(Date.now() / 1000) - connectedUserLastEventTime > 10) {
