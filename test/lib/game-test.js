...
describe('Game', () => {
  ...
  it('generates game cube location with each cube appearing once', () => {

    const cubeLocation = game.generateNewRound();
  ...
  it('can start a new round', (done) => {
    ...
    game.startNewRound(() => {
      ...
      game.currentGame((err, g) => {
        expect(g.p('round')).to.not.deep.equal([1, 2, 3, 0, 0, 0]);
        done();
      });
  ...
  it('can check win', (done) => {
    ...
    game.startNewRound(() => {
      game.currentGame((err, g) => {
        game.checkWin(g.p('round'), (err, win) => {
